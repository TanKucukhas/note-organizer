-- Update idea status values to match project statuses
-- Old: 'active', 'developing', 'shelved', 'trashed'
-- New: 'planning', 'in_progress', 'blocked', 'on_hold', 'completed', 'trashed'

-- First, update existing values to match new schema
UPDATE ideas SET status = 'in_progress' WHERE status = 'active';
UPDATE ideas SET status = 'planning' WHERE status = 'developing';
UPDATE ideas SET status = 'on_hold' WHERE status = 'shelved';
-- 'trashed' stays the same
