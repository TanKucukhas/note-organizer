...
export function createTask(input: CreateTaskInput): Task {
  ...
  db.prepare(`
    INSERT INTO tasks (
      ...
      project_id, idea_id, source_note_id
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    id,
    input.title,
...
export function createIdea(input: CreateIdeaInput): Idea {
  ...
  db.prepare(`
    INSERT INTO ideas (
      id, title, intro, description_md, status, category, source_note_id
    ) VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(
    id,
    input.title,
...
export function getAllIdeas(): Idea[] {
  const db = getOrganizationDatabase();
  return db.prepare<[], Idea>('SELECT * FROM ideas ORDER BY created_date DESC').all();
}
...
export function getAllProjects(): Project[] {
  const db = getOrganizationDatabase();
  return db.prepare<[], Project>('SELECT * FROM projects ORDER BY created_date DESC').all();
}
